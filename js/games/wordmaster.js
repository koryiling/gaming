...
Arcade.register({
  ...
  name: "Word Master",
  emoji: "🔤",
  tagline: "Guess the hidden word in six tries. Green = right spot, yellow = wrong spot.",
  ...
  rules: [
    ...
    "⬜ Grey = letter is not in the word. Crack it within 6 guesses!",
  ],
  options: [
  ...
  create(api) {
    ...
    boardEl.style.cssText = "display:grid;grid-template-rows:repeat(" + ROWS + ",1fr);gap:6px";
    const cellSize = Math.min(56, Math.floor((Math.min(420, window.innerWidth - 40)) / LEN) - 6);
